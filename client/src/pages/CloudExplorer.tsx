...
const PREMIUM_PROVIDERS: Provider[] = ['onedrive', 'box', 's3'];
...
function CloudPanel({
  ...
  function switchProvider(p: Provider) {
    if (isFree && PREMIUM_PROVIDERS.includes(p)) {
      toast({
        title: "Función exclusiva Pro",
        description: "OneDrive, Box y Amazon S3 requieren un plan Pro o Business.",
        variant: "destructive",
      });
      return;
    }
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-0.5 p-1 bg-white rounded-xl border border-gray-200 shadow-sm">
              {ALL_PROVIDERS.map(p => {
                const locked = isFree && PREMIUM_PROVIDERS.includes(p);
                return (
                <button
                  key={p}
                  onClick={() => switchProvider(p)}
                  title={locked ? `${getProviderName(p)} — Requiere Pro` : getProviderName(p)}
